...
<li class="mb-2"><c:set var="isProductActive"

				value="${currentMenu == 'product' || empty currentMenu}" /> <a

			href="${pageContext.request.contextPath}/admin/product/Ad_Product.jsp"

			class="d-block menu-link-item text-dark <c:if test="${isProductActive}">menu-link-active</c:if>">

				商品 </a></li>

		<li class="mb-2"><c:set var="isStoreActive"

				value="${currentMenu == 'store'}" /> <a

			href="${pageContext.request.contextPath}/admin/store/Ad_store.jsp"

			class="d-block menu-link-item text-dark <c:if test="${isStoreActive}">menu-link-active</c:if>">

				店舗 </a></li>

		<li class="mb-2"><c:set var="isRecipeActive"

				value="${currentMenu == 'recipe'}" /> <a

			href="${pageContext.request.contextPath}/admin/recipe/Ad_recipe.jsp"

			class="d-block menu-link-item text-dark <c:if test="${isRecipeActive}">menu-link-active</c:if>">

				料理 </a></li>
<li class="mt-2"><c:set var="isLogoutActive"

				value="${currentMenu == 'logout'}" /> <a

			href="${pageContext.request.contextPath}/admin/account/Ad_logout.jsp"
